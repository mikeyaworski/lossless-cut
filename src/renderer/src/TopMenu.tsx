...
import { CrossIcon, ListIcon, VolumeUpIcon, VolumeOffIcon, RefreshIcon } from 'evergreen-ui';
    ...
    >
      {filePath && (
        <>
          ...
          {numAudioTracks > 1 && (
            <Button
              title={t('Switch audio tracks')}
              onClick={toggleAudioTrack}
            >
              <><RefreshIcon size={'1em' as unknown as number} verticalAlign="middle" marginRight=".3em" />{t('Switch Audio Tracks')}</>
            </Button>
          )}
        </>
      )}
