...
const { focusWindow, quitApp, pathToFileURL, setProgressBar, sendOsNotification } = window.require('@electron/remote').require('./index.js');
...
function App() {
  ...
  const allStreams = Object.values(allFilesMeta).flatMap(({ streams }) => streams);
  ...
  const toggleAudioTrack = useCallback(() => {
    ...
    const activeIdx = Array.from(videoRef.current.audioTracks).findIndex(track => track.enabled);
